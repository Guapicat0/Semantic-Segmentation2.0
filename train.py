...
if __name__ == "__main__":
    ...
    #------------------------------------------------------------------#
    save_dir            = 'logs/pilelogs/SegNet/improved-SegNet'
    #------------------------------------------------------------------#
    ...
    if pretrained:
        if distributed:
            ...
            download_weights(backbone)
    #model = get_model_from_name[backbone](num_classes=2).train()
    model = get_model_from_name[SSeg](num_classes = 2,pretrained = False).train()
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
    ...
    if local_rank == 0:
        show_config(
            num_classes = num_classes, SSeg= SSeg,backbone = backbone, model_path = model_path, input_shape = input_shape, \
